...
namespace MainApplication
{
	...
	void Output::PrintLine(char symbol, bool onlyMaster)
	{
		...
		if (onlyMaster && ParallelEnvironment::Process().Rank() != 0)
			return;
	...
	void Output::PrintErrorMessage(const string& message, const bool& onlyMaster, ...)
	{
		...
		if(!onlyMaster)
			newMessage<< " on process "<< ParallelEnvironment::Process().Rank();
		newMessage<< endl;
	...
	void Profiler::StopTime(const string& nameTime, const bool& printTime)
	{
		...
		if (ParallelEnvironment::Process().Rank() == 0)
		{
			...
			outFile<< scientific<< ParallelEnvironment::Process().NumberProcesses()<< " "<< nameTime<< " "<< timeDifference<< endl;
		}
		...
		nameFile<< nameFolder.str()<< ConfigFile::Log().TimeFile()<< "_"<< ParallelEnvironment::Process().Rank()<< ".csv";
		bool fileExists = Output::FileExists(nameFile.str());
		...
		outFile<< scientific<< ParallelEnvironment::Process().NumberProcesses()<< " "<< nameTime<< " "<< localTimeDifference<< endl;
	}
	// *************************************************************************
	...
	void Profiler::CheckMemory(const string& nameMemory, const bool& checkProcessesMemory)
	{
		...
		if(ParallelEnvironment::Process().Rank() == 0)
		{
			...
			outFile<< ParallelEnvironment::Process().NumberProcesses()<< " "<<
								nameMemory<< " "<<
								totalProcessMemory<< " "<<
								totalAvailMemory<< " "<<
...
#if LOGGING==1 || LOGGING==3
		cout<< " -- "<< Output::BlueColor<< "Profiling"<< Output::EndColor<< ": RAM Memory '"<< nameMemory<<
					 "': Process "<< setfill(' ') << setw(3)<< ParallelEnvironment::Process().Rank() << " Used / Free: "<<
